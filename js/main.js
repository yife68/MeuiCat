document.addEventListener('DOMContentLoaded', function () {
  ...
  const sidebarFn = {
    open: () => {
      ...
      btf.animateIn(document.getElementById('menu-mask'), 'to_show 0.6s')
    ...
    close: () => {
      ...
      btf.animateOut(document.getElementById('menu-mask'), 'to_hide 0.6s')
  ...
  const addHighlightTool = function () {
    ...
    const highlightShrinkEle = isHighlightShrink !== undefined ? `<i class="iconfont icat-angle-down-new expand ${highlightShrinkClass}"></i>` : ''
    const highlightCopyEle = highlightCopy ? '<div class="copy-notice"></div><i class="iconfont icat-copy-paste copy-button"></i>' : ''
    ...
    function createEle (lang, item, service) {
      ...
      if (highlightHeightLimit && item.offsetHeight > highlightHeightLimit + 30) {
        ...
        ele.innerHTML = '<i class="iconfont icat-angle-double-down"></i>'
    ...
    $switchBtn && $switchBtn.addEventListener('click', function () {
      ...
      document.querySelectorAll('#post-comment > .comment-wrap > div').forEach(function (item) {
        if (btf.isHidden(item)) {
          item.style.cssText = 'display: block;animation: tabshow .6s'
        } else {
