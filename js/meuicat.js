function whenDOMReady() {
	"/privacy/" == location.pathname && meuicat.addScript("ua-ip", "https://cdn.meuicat.com/gh/UAParser.js/ua-parser.min.js", meuicat.getIpInfo());
	...
	if (location.pathname == '/album/') meuicat.photos('旅途');
	if (location.pathname == '/' || location.pathname.startsWith('/page/')) meuicat.listenToPageInputPress(), meuicat.homeplatform(), meuicat.Tagscolor(), meuicat.swiper(), meuicat.RandomPosts();
	if (location.pathname.startsWith('/blog/')) meuicat.post_cover(), meuicat.postplatform(), meuicat.postai();
	if (location.pathname.startsWith('/about/')) meuicat.Introduction(), meuicat.runtimen(), meuicat.fiftyonela();
	if (location.pathname == '/essay/') meuicat.changeTime(); meuicat.reflashWaterFall(); window.lazyLoadInstance && window.lazyLoadInstance.update(); meuicat.percent(); loadMeting();

	if (document.getElementById('post-comment')) meuicat.owoBig();
}
...
window.onscroll = meuicat.percent;
...
var icat = {
    ...
    homebannerInfo: function() {
        var socialBox = document.querySelector('.banner-bg');
        var socialElement = document.querySelector('.banner-btn');
